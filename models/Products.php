...
class Products extends model{
	...
	public function getInfoStore($id){
		...
		$sql = "SELECT products.id, products.cod_prod, products.name, store_stock.cod_prod,store_stock.id_product, store_stock.titulo, store_stock.estoque_qtd FROM products 
		INNER JOIN store_stock ON products.cod_prod = store_stock.cod_prod
	...
	public function getAvailableValuesFromOptions($groups,$ids){
		...
		foreach($groups as $op){
			$array[$op] = array('name' => $options->getName($op),'options' => array());
		}
	

		$sql = "SELECT p_value,id_option,COUNT(id_option) as c FROM products_options WHERE 
		id_option IN ('".implode("','",$groups)."') AND id_product IN ('".implode("','",$ids)."') GROUP BY p_value,id_option ORDER BY id_option";
	...
	public function getList($offset, $limit, $filters=array(), $random = false){
		$array= array();
		...
		FROM products WHERE ".implode(' AND ',$where)." ".$orderBySQL." LIMIT $offset, $limit";
	...
	public function getProductInfo($id){
		...
		if(!empty($id)){
			...
			$sql = "SELECT *,
			( select brands.name from brands where brands.id = products.id_brand ) as brand_name, ( select store_stock.estoque_qtd from store_stock where products.cod_prod = store_stock.cod_prod ) as total
	...
	public function getOptionsByProductId($id){
		...
		// Etapa 1 - pegar os nomes das opçoes
